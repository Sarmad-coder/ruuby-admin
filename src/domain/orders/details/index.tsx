...
const client = axios.create({
  baseURL: `https://store-api.ruuby.co`,
})

export const getOrderAttributes = async (id: string): Promise<any> => {
  console.log(`'${"order_01GXZKNTSTRMM71ZQ72BMMFVYR"}'`, `'${id}'`)
  const { data } = await client.get(`/get-order-by-id?id=${id}`)
  return data
}
...
const OrderDetails = () => {
  ...
  const [orderAttributes, setOrderAttributes] = useState<any>({})

  useEffect(() => {
    getOrderAttributes(order?.id as any).then((x) => {
      setOrderAttributes(x)
      console.log(x)
    })
  }, [order])
                ...
                >
                  ...
                  <div className="flex mt-6 space-x-6 divide-x">
                    <div className="flex flex-col">
                      ...
                      <div>{orderAttributes?.name || "N/A"}</div>
                    </div>
                    <div className="flex flex-col pl-6">
                      <div className="inter-smaller-regular text-grey-50 mb-1">
                        Acknowledgement
                      </div>
                      <div>{orderAttributes?.acknowledgement || "N/A"}</div>
                    </div>
                    <div className="flex flex-col pl-6">
                      <div className="inter-smaller-regular text-grey-50 mb-1">
                        Size
                      </div>
                      <div>{orderAttributes?.size || "N/A"}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex mt-6 space-x-6 divide-x">
                    <div className="flex flex-col">
                      ...
                      <div>{orderAttributes?.orientation || "N/A"}</div>
                    </div>
                    <div className="flex flex-col pl-6">
                      <div className="inter-smaller-regular text-grey-50 mb-1">
                        QR Code
                      </div>
                      <div>{orderAttributes?.qrCode || "N/A"}</div>
                    </div>
                    <div className="flex flex-col pl-6">
                      <div className="inter-smaller-regular text-grey-50 mb-1">
                        Image
                      </div>
                      <img className="w-20 cursor-pointer" src={orderAttributes?.logo} alt="" onClick={()=> {
                        const newTab = window.open();
                        (newTab?.document.body as any).innerHTML = `<img src="${orderAttributes?.logo}" alt="" />`
